 ...
 */
@Service
public class UserRolesDTS
{
	private static final Logger logger = LoggerFactory.getLogger( UserRolesDTS.class );
	private static final String DAOOBJ = "UserRolesDAO";
	private static final String TABLENAME = "user-roles";
	 ...
	 */
	public List< UserRolesDAO > getAllRows() throws NoRowsFoundException, DataAccessException
	{
		logger.info( "getAllRows: Started." );
		// Create SQL query.
		String sqlQuery = "SELECT * FROM " + TABLENAME;
		// Create results list.
		List< UserRolesDAO > rowsOutput = new ArrayList<>();
		// Exception catch.
		try
		{
			// Run SQL Query.
			...
			while ( srs.next() )
			{
				...
				rowsOutput.add( new UserRolesDAO( srs.getInt( IDCOL ), srs.getString( NAMECOL ) ) );
			}
			if ( !rowsFound )
			{
	 ...
	 * @throws DataAccessException  See message for reason for exception.
	 */
	public List< UserRolesDAO > search( String column, String query ) throws NoRowsFoundException, DataAccessException
	{
		...
		// Create results list.
		List< UserRolesDAO > searchOutput = new ArrayList<>();
		...
		try
		{
			...
			while ( srs.next() )
			{
				...
				searchOutput.add( new UserRolesDAO( srs.getInt( IDCOL ), srs.getString( NAMECOL ) ) );
			}
	 ...
	 */
	public boolean create( UserRolesDAO entry ) throws DataAccessException
	{
	 ...
	 */
	public boolean delete( UserRolesDAO entry ) throws DataAccessException
	{
	 ...
	 *                             updated.
	 */
	public boolean update( UserRolesDAO entry ) throws DataAccessException
	{
		logger.info( "update: Started." );
		// Create sql statement
		String sqlQ = "UPDATE " + TABLENAME + " SET " + IDCOL + " = ?, " + NAMECOL + " = ?, WHERE " + IDCOL + " = ?";
