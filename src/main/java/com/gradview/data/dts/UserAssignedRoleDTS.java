 ...
 */
@Service
public class UserAssignedRoleDTS
{
	private static final Logger logger = LoggerFactory.getLogger( UserAssignedRoleDTS.class );
	private static final String DAOOBJ = "UserAssignedRoleDAO";
	private static final String TABLENAME = "`user-assigned-role`";
	 ...
	 */
	public List< UserAssignedRoleDAO > getAllRows() throws NoRowsFoundException, DataAccessException
	{
		logger.info( "getAllRows: Started." );
		// Create SQL query.
		String sqlQuery = "SELECT * FROM " + TABLENAME;
		// Create results list.
		List< UserAssignedRoleDAO > rowsOutput = new ArrayList<>();
		// Exception catch.
		try
		{
			// Run SQL Query.
			...
			while ( srs.next() )
			{
				...
				rowsOutput.add( new UserAssignedRoleDAO( srs.getInt( USERIDCOL ), srs.getInt( ROLEIDCOL ) ) );
			}
			if ( !rowsFound )
			{
	 ...
	 * @throws DataAccessException  See message for reason for exception.
	 */
	public List< UserAssignedRoleDAO > search( String column, String query )
			throws NoRowsFoundException, DataAccessException
	{
		...
		// Create results list.
		List< UserAssignedRoleDAO > searchOutput = new ArrayList<>();
		...
		try
		{
			...
			while ( srs.next() )
			{
				...
				searchOutput.add( new UserAssignedRoleDAO( srs.getInt( USERIDCOL ), srs.getInt( ROLEIDCOL ) ) );
			}
	 ...
	 */
	public boolean create( UserAssignedRoleDAO entry ) throws DataAccessException
	{
		logger.info( "create: Started." );
		// Create sql statement.
		String sqlQ = "INSERT INTO " + TABLENAME + "(" + USERIDCOL + ", " + ROLEIDCOL + ") VALUES(?, ?)";

		try
		{
			// create
			int rows = this.jdbcTemplate.update( sqlQ, entry.getUserID(), entry.getRoleID() );
			if ( rows == 1 )
			{
	 ...
	 */
	public boolean delete( UserAssignedRoleDAO entry ) throws DataAccessException
	{
		logger.info( "delete: Started." );
		// Create sql statement.
		String sqlQ = "DELETE FROM " + TABLENAME + "WHERE " + USERIDCOL + " = ? AND '" + ROLEIDCOL + "' = ?";
		try
		{
			// delete
			int rows = this.jdbcTemplate.update( sqlQ, entry.getUserID(), entry.getRoleID() );
